...
public class PMate extends PApplet { //DONT REMOVE FOR JAVADOC
...
public void Start() {
    ...
    timer.startTimer(settings_json.getInt("trial_length"));
...
public void TestValve() {
    println("TEST VALVE");
    //TODO
    //int pin = Integer.parseInt(cp5.get(Textfield.class, "Test Pin").getText());
    int pin = 7;
    //int duration = Integer.parseInt(
    //    cp5.get(Textfield.class, "Duration").getText());
    int duration = 200;
...
void reload_settings() {
    ...
    configure_rewards();
    display.setTestPin(reward_valve);
...
/**
 * built in processung function with is run everytime a keyboard key is pressed.
 * The ENTER key starts an experiment and the 'q' ends the current trial. NOTE:
 * pressing ESC program will cause pMate to exit
 */
public void keyPressed() {
    if ((key == ENTER) && (!started)) {
        Start();
    } else if ((key == 'q') && (started)) {
        endExperiment();
    }
}
...
private void endExperiment() {
    ...
    //display.start_button.remove();
    //display.addStart();
    //display.refresh_button.remove();
    //display.addRefresh();
    //cp5.show();
